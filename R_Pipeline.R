...
setwd("/storage/hpc/data/bourasm/CXCR5/CXCR5_Retina")
CXCR5<-read.table("C57_vs_CXCR5_Retina_results.txt", header = TRUE, row.names = 1)
...
rwname<-attributes(resSig)[2]
resSig_results<-data.frame(attributes(resSig)[4], 
                           row.names = rwname[[1]])
write.table(resSig_results, "resSig_results.txt", sep=";")
...
# order by padjusted and print the top 5
attributes(res)
